...
(function (Hogan) {
  ...
  Hogan.stringify = function(codeObj, text, options) {
    return "{code: function (c,p" + (!options.minify ? ",i" : "") + ") { " + Hogan.wrapMain(codeObj.code, options) + " }," + stringifyPartials(codeObj) +  "}";
  }
  ...
  Hogan.wrapMain = function(code, options) {
    return 'var t=this;' + (!options.minify ? 't.b(i=i||"");' : '') + code + 'return t.fl();';
  }
  ...
  Hogan.makeTemplate = function(codeObj, text, options) {
    ...
    if (options.minify) {
      template.code = new Function('c', 'p', this.wrapMain(codeObj.code, options));
    } else {
      template.code = new Function('c', 'p', 'i', this.wrapMain(codeObj.code, options));  
    }
  ...
  Hogan.codegen = {
    '#': function(node, context, options) {
      context.code += 'if(t.s(t.' + chooseMethod(node.n) + '("' + esc(node.n) + '",c,p,1),c,p,0)){' +
    ...
    '^': function(node, context, options) {
      context.code += 'if(!t.s(t.' + chooseMethod(node.n) + '("' + esc(node.n) + '",c,p,1),c,p,1)){';
      Hogan.walk(node.nodes, context, options);
      context.code += '};';
    },
    ...
    '_t': function(node, context, options) {
      if (options.minify) {
        ...
        if (matches) {
          ...
          context.code = context.code.substr(0, context.code.length - matches[0].length);
        }
      }
  ...
  Hogan.cacheKey = function(text, options) {
    return [text, !!options.asString, !!options.disableLambda, options.delimiters, !!options.modelGet, !!options.minify].join('||');
  }
